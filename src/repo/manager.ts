...
export class RepoManager {
  ...
  async prepareRepository(input: PrepareRepositoryInput): Promise<PreparedRepository> {
    ...
    if (await pathIsDirectory(checkoutPath)) {
...
async function pathIsDirectory(path: string): Promise<boolean> {
  ...
  try {
    ...
    throw new RepoManagerError(`Unable to inspect repository checkout path: ${path}`, {
      cause: err
    });
  }
  if (entry.isDirectory()) return true;
  throw new RepoManagerError(`Repository checkout path is not a directory: ${path}`);
}
